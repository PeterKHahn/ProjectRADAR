 ...
 *
 * @param <T> The type of element that the KDTree's Nodes hold
 * @author efu2
 */
public class KDTree<T extends DimensionsComparable<T>
    & DimensionsQueryable<Double> & DistanceCalculatable<T>> {
  private Node<T> root;
   ...
   *
   * @param elements A List of elements to add to the KD Tree
   * @param dimensionNum The number of dimensions that each instance of T has
   */
  public KDTree(List<T> elements, Integer dimensionNum) {
    if (dimensionNum < 1) {
      System.out.println("ERROR: Number of dimensions must be positive.");
      return;
    }
  ...
  private void addChild(
          List<T> sorted, Node<T> parent, boolean isLeftChild, int dimension) {
    ...
    if (sorted.size() > 0) {
      ...
      int nextDimension = (dimension + 1) % this.dimensionNum == 0
          ? this.dimensionNum : (dimension + 1) % this.dimensionNum;
   ...
   * the given dimension.
   *
   * @param sorted List of elements to add to the KD Tree, sorted by the given
   * dimension
   * @param leftElements List to build on that only contains elements less than
   * the median at the given dimension
   * @param rightElements List to build on that only contains elements greater
   * than the median at the given dimension.
   * @param dimension The next dimension to sort by.
   */
  private void halveList(
          List<T> sorted, List<T> leftElements, List<T> rightElements,
          int dimension) {
    int numberOfElements = sorted.size();
   ...
   * @param center The object to compare distances to.
   * @param excludeCenter Whether or not the center element should be excluded
   * in the neighbors
  ...
  public List<T> kNearestNeighbors(int k, T center, boolean excludeCenter) {
    if (k > 0) {
      ...
      this.neighborsHelper(
          this.root, center,
          nearest, distances,
          Double.MAX_VALUE, excludeCenter,
          true);
      return nearest;
    }
   ...
   *
   * @param r2 The squared radius around the center.
   * @param center The object to compare distances to.
   * @param excludeCenter Whether or not the center element should be excluded
   * in the neighbors
   ...
   *
   * @param currNode The Node to start traversing at.
   * @param center The element whose position to calculate distances from.
   * @param neighbors A sorted List of nearest neighbors found.
   * @param distances A List of distances corresponding to elements in
   * {@code neighbors}.
   * @param distanceThreshold The distance that determines whether currNode
   * is added to {@code neighbors}.
   * @param excludeCenter Whether or not to exclude the center in
   * {@code neighbors}.
   * @param keepOnlyK Whether to keep only k nearest neighbors or all
   * neighbors within {@code distanceThreshold}.
   */
  private void neighborsHelper(
      Node<T> currNode, T center,
      List<T> neighbors, List<Double> distances,
      double distanceThreshold, boolean excludeCenter,
      boolean keepOnlyK) {
    if (currNode == null) {
      return;
    }
    ...
    if (currNode.getLeftChild() != null
      ...
      if (searchLeftFirst) {
        ...
        if (center.getDimension(splitDimension) - distanceThreshold
            <= currNode.getElement().getDimension(splitDimension)) {
          neighborsHelper(
              currNode.getLeftChild(), center,
              neighbors, distances,
              newThreshold, excludeCenter,
              keepOnlyK);
        }
  ...
  /**
   * Inserts an object with a distance smaller than that of the current
   * nearest neighbors to the nearest neighbors List.
   *
   * @param elementToAdd The element to add to the nearest neighbors.
   * @param distance The distance to the elementToAdd.
   * @param neighbors The List of neighbors.
   * @param distances The List of distances to each nearest neighbor.
   * @param keepOnlyK Whether to keep only k nearest neighbors.
   */
  private void insertNeighbor(
      T elementToAdd, double distance,
      List<T> neighbors, List<Double> distances,
      boolean keepOnlyK) {
    int index = 0;
